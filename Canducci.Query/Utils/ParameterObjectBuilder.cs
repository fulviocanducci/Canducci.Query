...
namespace Canducci.Query.Utils
{
    internal static class ParameterObjectBuilder
    {
        ...
        {
            var assemblyBuilder = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("QueryBuilder.ParameterObjects"), AssemblyBuilderAccess.RunAndCollect);
            ModuleBuilder = assemblyBuilder.DefineDynamicModule("MainModule");
        }

        public static object CreateObjectWithValues(object[] parameters)
        {
            IDictionary<string, object> dic = new Dictionary<string, object>();
            foreach(var parameter in parameters)
            {
                dic.Add(dic.Count.ToString(), parameter);
            }
            return CreateObjectWithValues(dic);
        }
